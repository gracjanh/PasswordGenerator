...
namespace GeneratorHasla
{
    public partial class Form4 : Form
    {
        ...
        {
            bool zawieraPopularne = false;

            // textBox1 nie może być pusty
            if (textBox1.Text != "")
            {
                string[] popularne_hasla = File.ReadAllLines(Directory.GetCurrentDirectory() + "/pliki_tekstowe/popularne_hasla.txt");
                HashSet<string> popularneSet = new HashSet<string>(popularne_hasla, StringComparer.OrdinalIgnoreCase);
                ...
                if (popularneSet.Contains(textBox1.Text, StringComparer.Ordinal))
                {
                    zawieraPopularne = true;
                }


                if (zawieraPopularne)
                {
                    MessageBox.Show("Hasło jest zbyt popularne! Wprowadź inne.", "Błąd", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
                else
                {
                    // Szyfrowanie
